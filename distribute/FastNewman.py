...
def fast_newman(node_list, adjacent_matrix, divide_num):
    ...
    while len(list_unique(club_list)) != 1:  # 计算满足条件的个数
        ...
        for item in list_unique(club_list):
            ...
            result.append([node_list[t] for t in tmp])

        if len(result) <= divide_num:
            break
    return result
...
if __name__ == '__main__':
    ...
    printf(fast_newman(node_list, create_relation_matrix(List, 6), 6))
